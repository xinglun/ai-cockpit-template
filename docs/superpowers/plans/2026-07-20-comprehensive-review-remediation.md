...
- 已确认本轮已完成的改进：默认 Enforced Preflight、Raw User Request 字段、中文/日文/英文已知风险短语、Trust Layer Stop Demo、企业安全边界与模板导入稳定性文档、上一版发布、Adoption Smoke 修复。
...
**验收：** 状态唯一经过 `development → candidate_prepared → candidate_verified → release_published`；canonical 文件至少记录 `state`、`releaseTag`、`sourceCommit`、`previousRelease`、`evidenceBundleDigest`；失败不发布、不移动 Tag、不删除证据；既有发布证据可兼容读取或有明确迁移记录。
