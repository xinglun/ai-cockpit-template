...
def main(
    ...
    if not isinstance(capabilities, dict):
        return _fail("release.json capabilities must contain an object")
    archive_verification = capabilities.setdefault("sha256ArchiveVerification", {})
    if not isinstance(archive_verification, dict):
        return _fail("release.json sha256ArchiveVerification must contain an object")
    archive_verification.update(
        supported=True,
        verified=True,
        status="verified",
        reason="Canonical archive and installer bindings were generated by release finalization.",
    )
    ...
    # Persist the symbolic ref across the later manifest commit; preflight resolves identities.
