...
This is controlled fixture evidence. It does not prove enterprise identity, authorization, immutable audit, regulatory compliance, provider workflow behavior, or production readiness. Run `make cross-stack-long-cycle` to aggregate Python and Java dependency-free fixture phases with the isolated adopter Git lifecycle. The TypeScript fixture has an additional real npm lifecycle test: its `lifecycle.mjs` executes install, build, test, lint, format, and local upgrade/rollback transitions, and records `executionKind` as `local_real_execution`, `blocked`, or `not_run`. The Java fixture executes two-module `javac`/`java` compilation and tests plus a temporary upgrade/rollback comparison; missing JDK/Maven and provider operations remain `not_run`. The generic `fixture_harness.py` remains deterministic manifest simulation and must not be cited as real fixture execution. Install, Configure, Normal Work Item, Ambiguous Request, Critical Domain Change, Upgrade, Rollback, and Release Check remain explicit phases; blocked phases include resume conditions. Provider and identity evidence remain `not_run`, and enterprise assurance remains `not_claimed`.
