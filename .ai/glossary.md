...
| no_active_work_item | active Contract/Summary が存在しない安定状態。worktree が無変更であることは意味せず、瞬時の Git diff は状態へ保存しないが、no-active status は `Worktree Changes` と `Ownership Preview` の圧縮信号を残す。アーカイブ済み証跡と完全 PR diff の帰属は `make check-ai-pr AI_BASE_COMMIT=<merge-base>` が決め、重複する archive claim は最後に有効だった pair を採用する。 |
