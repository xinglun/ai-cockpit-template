...
The historical release tag is immutable evidence and is not rewritten. A release-preparation PR may declare exactly the next patch tag as pending publication; it must pass local metadata and evidence checks without claiming that the public tag already exists. After the PR merges, maintainers dispatch `.github/workflows/release.yml` with the exact verified `main` SHA. That workflow creates the immutable tag and GitHub Release, then dispatches a strict smoke verification against the now-public release. The workflow requires `release.json.releaseTag` to match the requested tag. The Candidate PR's preparation snapshot is not the release source of truth. After the PR merges, the workflow resolves the remote symbolic default branch, fetches its current commit, and sets `SOURCE_COMMIT` to that commit. A supplied `source_commit` is only an equality assertion; a stale value fails before checkout, evidence generation, tag creation, Draft Release creation, publication, or any evidence cleanup. The workflow therefore uses the post-merge automatic-resolution model and does not require freezing `main`.
