...
The premerge finalizer calculates `sourceTree` and `archiveSha256` from the clean candidate `HEAD`, not the still-old remote default branch. Hosted release preflight resolves the controlled source reference after PR merge, checks out that exact commit detached, and recalculates canonical content. If candidate and merged calculations disagree, publication stops before tag mutation and the process must be corrected before retry.
