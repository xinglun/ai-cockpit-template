...
AI Cockpit は **汎用テンプレート + ローカルキャリブレーション** モデルに適しています。インストールは可能ですが、本番 readiness には導入後の `configure_ai_cockpit` Work Item が必要です。
...
## ローカルキャリブレーション checklist

1. **Adopt → Configure:** `adopt_ai_cockpit` を単独 commit で完了し、続けて `configure_ai_cockpit` で Profile、Guard、品質コマンド、CI を適応する。
...
9. 必要なら quality を optional にした试点 Work Item を実行し、その後 quality と Coverage を blocking ゲートに昇格する。
