...
| 13 | `task-outcome-schema` | Outcome JSON Schema and binding model | Work Item 12 closure |
| 14 | `task-event-log` | Append-only Task lifecycle events, fingerprints, and deduplication | Work Item 13 closure |
| 15 | `task-outcome-generator` | Evidence-derived Outcome JSON generator | Work Item 14 closure |
| 16 | `task-outcome-validator` | Schema, binding, provenance, privacy, and claim validator | Work Item 15 closure |
| 17 | `task-outcome-markdown-renderer` | Derived human-readable report renderer | Work Item 16 closure |
| 18 | `task-outcome-ai-finish-integration` | Final verification, generation, validation, and Cockpit link integration | Work Item 17 closure |
| 19 | `task-outcome-archive-integration` | Active/archive lifecycle integration | Work Item 18 closure |
| 20 | `task-outcome-pr-summary` | Sanitized PR Outcome Summary integration | Work Item 19 closure |
| 21 | `task-outcome-multilingual` | `ja`, `en`, and `zh-CN` rendering controlled by Project Profile | Work Item 20 closure |
| 22 | `task-outcome-documentation` | Feature/maintainer documentation and final Outcome self-check | Work Item 21 closure |
| 23 | `publish-new-version` | Strict final release verification, actual new-version publication, post-publication verification, and closure | Work Item 22 closure |
...
## Plan Extension: AI Cockpit Task Outcome Report
This appended extension serves `Calibrated Human-Agent Trust` and does not rewrite earlier Work Items. Keep Cockpit Status as the short “can execution continue?” view (signal, allow/stop, human confirmation, blockers, Evidence integrity); keep Task Outcome Report as the evidence-backed “what value did AI Cockpit provide?” view. Never copy the full report into `current_status.md`, manufacture praise/scores/productivity claims, or use finding/intervention counts as performance targets.
### Responsibilities, source, and report structure
Machine truth is `.ai/work-items/active/<task>.outcome.json`; Markdown is derived at `.ai/work-items/active/<task>.outcome.md`; archive equivalents are `outcome.json` and `outcome.md`. Rebuild from Contract, Summary, Verification, Guards/Risks, Checkpoints, Human Confirmations, Stop/Resume, changed files, tests, reviews, commit/PR binding, and archive manifest. Contract/Summary/Verification changes expire the report. Always render Outcome Summary, Task Overview, Delivered Changes, Findings, Risks, Warnings, Interventions, Forced Stops, Resolutions, Recurrence Prevention, Avoided Impact, Residual Risks, Human Decisions, and Evidence; empty sections say `None`. Final statuses are `completed`, `completed_with_warnings`, `needs_human_confirmation`, `blocked`, and `cancelled`.
Findings support the requested gap/defect/evidence/security/installer/release/process categories plus `other`; severities are informational/low/medium/high/critical; final states are resolved/mitigated/accepted/unresolved/not_applicable. Risks must distinguish observed problem, potential risk, and prevented event. Interventions are observed/warned/intervened/prevented. Forced Stops record time, stage, reason, Policy/Guard, attempted action, conditional impact, handoff, human decision, recovery/resume, and result. Resolutions link `Problem → Action → Verification → Result`. Recurrence prevention is None, Documentation, Test, Automated Check, or Structural Prevention and states coverage/limits/human dependency.
Avoided Impact requires Finding/Risk/Intervention/Stop/Resolution/Test Evidence and conditional language (`If not detected`, `Could have`, `如果未被发现，可能导致`). Do not invent hours, money, percentages, “major disaster” claims, Trust/Safety scores, or combined scores. Human decisions must remain distinct from Agent decisions; unresolved and accepted residual risks cannot be hidden.
### Data model, events, generator, validator, and integration
Add `.ai/schemas/task_outcome.schema.json` with empty arrays valid and bind Task ID, Contract/Summary/Verification digests, base/head commits, PR, AI Cockpit and generator versions. Add append-only `.ai/work-items/active/<task>.events.jsonl` for finding/risk/warning/confirmation/stop/resume/resolution/risk-accepted/check-pass-after-fix/prevention/completed/cancelled events; corrections use `event_corrected`/`event_superseded`, never silent edits. Deduplicate by `findingFingerprint` from Checker ID, reason code, affected resource, and Evidence subject; a post-fix recurrence is new.
Add `scripts/ai_generate_task_outcome.py` to read structured Evidence, deduplicate Findings, link Resolutions, derive prevention/conditional Avoided Impact/Residual Risks, emit JSON, and render Markdown. Add `scripts/ai_check_task_outcome.py` to validate Schema, bindings, provenance, event relationships, JSON/Markdown parity, privacy, conditional claims, no-score/no-unsupported-quantification rules, and visibility of residual risk. `ai-finish` order is Final Verification → Summary generation/validation → Outcome generation/validation → Markdown → Cockpit Status → presentation → archive. Validation failure preserves raw Evidence and cannot claim success. Status adds only a short link/count summary; PR adds only Project-Profile-approved sanitized Outcome Summary.
### Language, privacy, tests, docs, and acceptance
Project Profile controls `reporting.defaultLanguage` (`ja`, `en`, `zh-CN`) and whether `outcome.ja.md`, `outcome.en.md`, and `outcome.zh-CN.md` are generated; JSON keys remain English and one fact source is maintained. Test minimum/complete/empty reports, invalid bindings/status/severity/level, deduplication/recurrence, every stop outcome, Evidence-backed versus unsupported Avoided Impact, conditional language, prevention levels, human decisions, Markdown parity, multilingual rendering, empty sections, privacy, no-score, and all five final statuses. Add `docs/features/task-outcome-report.md` and `docs/maintainers/task-outcome-events.md` covering purpose, separation, structure, Evidence, categories, stops, resolutions, recurrence, residuals, privacy, language, events, fingerprints, and anti-exaggeration. Outcome feature completion requires all Work Items merged, tests/bindings/reconstruction/multilingual/PR/archive checks, final self-check, no hidden risks or unsupported claims, and user confirmation; until then the feature state is `needs_human_confirmation`.
### Final release Work Item — `publish-new-version`
This is the last Work Item of the entire plan, after Work Items 0–22, issue overview, Outcome self-check, known-gap review, and plan-document cleanup. It alone performs actual new-version publication. Its Contract requires explicit release identity/source/tag/asset/distribution/release-note data and human publication authorization. It runs strict Release Gate, Identity and Source/Tag/Asset binding, reproducibility, SBOM, provenance, vulnerability/secret, installer lifecycle, Public Install, Cross-stack, compatibility, ecosystem, preflight, Evidence Digest, and publication preconditions; any failure/unknown high-risk item/mismatch/missing authorization/verifier error is fail closed. After strict proof it may tag and publish, verify identity/assets/distribution/fresh install/update/rollback/disable-enable/uninstall proposal/public install, record URL/version and Evidence, complete PR/merge/`make ai-close-work-item`, clean branches, sync default branch, and report final user-review status. No earlier Work Item publishes and no later Work Item exists.

## Stop and Human Confirmation Gates

The following are already authorized: update and validate the execution plan so that the issue-record mechanism is Work Item 1, the lightweight verification and soft-gate refactor is Work Item 2, the installation improvement starts at Work Item 3, the Task Outcome Report extension follows Work Item 12, and actual new-version publication is Work Item 23 and the final task. The user has also authorized serial execution of every Work Item, with full PR and closure lifecycle, followed by a final issue overview document for user review.
...
- User review condition: all Work Items must close serially; Work Item 11 must produce the initial complete issue overview, Work Item 22 must produce the Outcome self-check, and Work Item 23 must append final release evidence and the final issue overview. The user reviews the issue document after Work Item 23 closes. Completion does not require retaining `needs_human_confirmation` as the final plan state for the plan itself.
