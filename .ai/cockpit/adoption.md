...
The installer also writes `.ai/cockpit/adoption-runtime-verification.json`. This record binds the adopter Contract, Summary, and Start Receipt, records the source release identity and actual check results, and keeps unavailable project-specific checks as `not_run`. It explicitly records `projectQualityState: not_configured`, `readiness: not_ready`, and `enterpriseAssurance: not_claimed`; those states remain until the later `configure_ai_cockpit` Work Item. Template-owned SBOM, provenance, release-digest, and baseline files are never valid adopter evidence.

When that Runtime Verification file exists, `make ai-start TASK=<ordinary-task> ...` stops with `Configuration Required` until the project reaches Adoption Ready. The reserved `configure_ai_cockpit` Work Item remains startable so the adopter can select and confirm its Profile, Guards, quality commands, and CI. The template-maintenance repository and repositories without adopter evidence are not gated by this rule.
