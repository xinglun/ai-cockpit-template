...
def prepare_work_item(tmp_path: Path, *, archive_collision: bool = False):
    ...
    installer = Installer(
        ...
        with_examples=False, update_makefile=False,
    )
    ...
    collision_path = tmp_path / ".ai/work-items/archive" / str(__import__("datetime").datetime.now().year) / "e2e.contract.json"
...
def test_required_failure_keeps_active_and_retry_archives(tmp_path):
    ...
    failed = run(tmp_path, "make", "ai-finish", "TASK=e2e", f"PYTHON={sys.executable}", "PROJECT_TEST=false")
    assert failed.returncode != 0
    assert contract_path.exists()

    retried = run(tmp_path, "make", "ai-finish", "TASK=e2e", f"PYTHON={sys.executable}", "PROJECT_TEST=true")
...
def test_archive_collision_fails_after_checks_and_preserves_active(tmp_path):
    ...
    result = run(tmp_path, "make", "ai-finish", "TASK=e2e", f"PYTHON={sys.executable}", "PROJECT_TEST=true")
