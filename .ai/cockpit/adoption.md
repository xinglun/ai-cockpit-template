...
5. Replace every placeholder in `Makefile.ai.stack` and confirm `make ai-cockpit-quality` passes.
6. Review the Coverage paths, then set `adoptionReviewed: true`.
7. Configure CI to fetch full Git history and run `make check-ai-pr AI_BASE_COMMIT=<merge-base-sha>` and `make ai-cockpit-quality` as independent required checks.
