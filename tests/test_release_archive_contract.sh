...
git archive --format=tar --mtime='1970-01-01 00:00:00' --prefix=ai-cockpit/ HEAD^{tree} \
  | python3 -c 'import gzip,sys; data=sys.stdin.buffer.read(); out=gzip.GzipFile(fileobj=sys.stdout.buffer, mode="wb", compresslevel=9, mtime=0); out.write(data); out.close()' \
