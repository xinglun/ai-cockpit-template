...
Runtime のインストールは校正の完了を意味しません。現在の `configure_ai_cockpit` は主に Project Profile の提案を生成・検証します。中断・再開できる 10 Stage セッションと Candidate 有効化は計画中の能力です。更新時は Impact Assessment を先に実行します。証拠の境界は [Capability Truth Matrix](docs/reference/capability-truth-matrix.md) を参照してください。
