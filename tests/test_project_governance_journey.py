...
def prepare_work_item(root: Path, task: str, changed: list[str], *, extra_checks=()) -> None:
    ...
    contract.update({
        ...
        "unknowns": [], "notCodable": False,
        "riskAssessment": {"level": "medium", "riskTypes": ["journey"], "reason": "Black-box journey fixture."},
        "agentCapability": {"canImplement": True, "canVerify": True, "needsHumanDecision": False, "blockedReason": ""},
        "executionDecision": {"status": "continue", "reason": "Fixture decisions are explicit."},
        ...
        "restrictedWriteApproval": {"approved": True, "approvedBy": "journey fixture owner", "reason": "Explicit fixture calibration."},
    })
    ...
    summary.update({
        "changedFiles": [
            {"path": f".ai/work-items/active/{task}.contract.json", "reason": "Journey Contract."},
            {"path": f".ai/work-items/active/{task}.summary.json", "reason": "Journey Summary."},
        ...
        "residualRisks": [],
        "reviewReadiness": {"status": "ready", "reason": "Fixture inputs are explicit.", "expectedReviewFocus": ["Journey ownership"]},
        "boundaryChecks": {"runtimeEntrypoints": "verified", "userVisibleOutput": "verified", "persistence": "not_applicable", "localization": "not_applicable", "generatedArtifacts": "verified", "makeEntrypoints": "verified"},
        "checkpointEvidence": [
            {"stage": stage, "recorded": True, "detail": "Journey checkpoint.", "contractHash": contract_hash, "acceptanceCount": 1, "unknownCount": 0, "requiredChecks": len(contract["verification"]), "requiredChecksPassed": 0}
...
def test_documented_project_governance_journey_and_upgrade_rollback(tmp_path):
    ...
    install = run(project, str(ROOT / "install.sh"), "--stack", "python", "--update-makefile", "--create-adoption", env=env)
    ...
    upgrade = run(project, str(ROOT / "install.sh"), "--stack", "python", "--update-makefile", "--upgrade", env=env)
    assert upgrade.returncode == 0, upgrade.stdout + upgrade.stderr
    assert (project / ".ai" / "project_profile.yaml").read_bytes() == profile_before

    broken_source = tmp_path / "broken-source"
    shutil.copytree(ROOT, broken_source, ignore=shutil.ignore_patterns(".git", ".venv", "target", "__pycache__"))
    broken_module = broken_source / "scripts" / "ai_check_scope.py"
    broken_module.write_text(broken_module.read_text(encoding="utf-8") + "\nthis is invalid python !!!\n", encoding="utf-8")
    ...
    failed = run(project, str(broken_source / "install.sh"), "--stack", "python", "--update-makefile", "--upgrade", env=failed_env)
