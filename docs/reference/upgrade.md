...
An installed repository receives the same Runtime Surface as the template: version/lifecycle facts, update check/proposal/apply, rollback proposal, disable/enable transitions, and preserve-evidence uninstall proposal. The installer copies the corresponding scripts and exposes them through `Makefile.ai`; if any script or target is missing, installation validation must fail closed. This parity does not imply that Calibration, Quick Install archive digest verification, or enterprise assurance has run.
