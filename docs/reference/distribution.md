...
Release progression is recorded in canonical `release-state.json`: `development` → `candidate_prepared` → `candidate_verified` → `release_published`. It carries the release tag, source commit, previous release, evidence bundle digest, and SHA-256 references to the legacy published/candidate metadata. `release.json` remains the published installer contract and `next-release.json` remains the unpublished candidate projection; `make check-release-state-consistency` requires their tags, previous-release reference, candidate state, and metadata digests to agree. Failed verification must leave the state and tag unchanged.
