...
Run `make check-governance-complexity` during maintenance and before a release. The check writes `target/governance_complexity_report.json` and reports tracked-file count, Python/Markdown line counts, archive Contract/Summary counts, and archive index consistency. Use `PYTHONDONTWRITEBYTECODE=1 .venv/bin/python scripts/ai_archive_work_item.py --rebuild-index` when a historical index needs to be regenerated from authoritative archive pairs; this command only updates the generated discovery index. An index repair is a restricted, explicitly approved Work Item change; Contract/Summary evidence remains append-only. Archive counts are observational metrics; they are not cumulative blocking limits.
