...
If an immutable tag is found to be invalid after creation, do not move, delete, or republish it. Quarantine any associated GitHub Release as Draft, record the failed evidence, repair the generator and pre-publication gates through a Work Item and PR, then publish the next patch as the corrective release. Release preparation must validate the local installer digest even while that next patch tag is still pending.
...
The v0.5.40 corrective preparation supersedes the quarantined, immutable v0.5.39 tag without rewriting it and keeps v0.5.41 as the distinct unpublished candidate. The v0.5.40 tag, provider assets, Workflow Run, and final evidence bundle remain pending until the post-merge release workflow resolves the latest default-branch commit and verifies them.
