...
| 工单 | 实际状态 | PR / 发布证据 |
| --- | --- | --- |
| 1 One Release Truth | **已补执行并关闭** | PR #237，合并 SHA `77ee94e6586de790d9c61c9493812037106c0a4d`；已执行 `ai-close-work-item`，并完成 canonical/projection 影响复核 |
| 2–10 | **已完成并关闭** | 对应 PR #225–#231；各自 Contract/Summary 已归档，PR 已合并，分支已清理 |
| 独立预算修复 | **已完成并关闭** | PR #238，合并 SHA `17b703c410c3c157f242e6d173b9575f57945260`；用户已授权的独立 Work Item，修复复杂度预算后完整关闭 |
...
| 13 发布新版本 | **已完成并关闭** | PR #239，合并 SHA `2b1cbe03144d3e6f585eb6df457a6ffd25ea4606`；Run `29922509671` 成功，v0.5.38 Tag/Release/资产已核验，并已执行 `ai-close-work-item` |
| 14 清理执行计划文档 | **已完成（关闭证据已归档）** | PR #236 已合并关闭；本 Work Item 已把全部编号、SHA、Run、Tag 和关闭证据固化到计划，PR/合并/分支清理详情保存在本 Work Item 归档 Summary |

此前版本的发布前 exact-SHA Smoke、Compatibility 和发布后严格 Smoke 均已成功；这些证据只作为历史发布证据，不替代工单1完成后的新版本发布证据。工单1补执行完成后，已重新运行受影响的状态、分发、供应链和 Release Workflow 检查，并以 v0.5.38 建立最终发布事实。

### 工单1补执行后的最终发布证据

v0.5.38 已由 PR #239 合并后的 main SHA `2b1cbe03144d3e6f585eb6df457a6ffd25ea4606` 发布。Release Workflow Run `29922509671` 成功；发布 Tag 指向该 SHA，Release 为非 Draft，资产已核验包含 `sbom.json`、`provenance.json`、`release-digests.json` 和 `release-source.json`。PR #239 的 exact-SHA Smoke、Compatibility、严格 Smoke、供应链和质量检查均通过。该证据不改变企业级安全/合规 NO-GO 边界。

### 工单1补执行影响评估

| 影响范围 | 结论 | 对应动作 |
| --- | --- | --- |
| 已发布的历史版本标签、Release 和资产 | 不回改，继续作为历史证据 | 保留原 PR #235、Run `29915996900`、标签和资产；v0.5.38 使用新 Tag |
...
| 下一版本 | 已重新发布 | 工单1合并关闭后创建独立发布 Work Item，已完成 Tag、Detached Checkout、Run、SBOM、Provenance、Digest 和 `ai-close-work-item` |
...
### 工单 13：发布新版本（P1，已在工单1后重新执行）

工单1补执行并合并关闭后，从最新远端默认分支重新发布 v0.5.38；此前历史版本仅作为历史发布。已重新绑定 Tag、Detached Checkout、`release.json`、Asset、Workflow Run、SBOM、Provenance 和 Digest，且未改变企业级安全/合规 NO-GO 结论。发布工单自身已再次走第 3 节完整流程。

### 工单 14：清理执行计划文档（P2，最终文档闭环）

在全部整改和新版本证据归档后，逐份审查计划，标记“执行中 / 已完成需保留 / 已被替代 / 可安全删除”，记录对应 Work Item/PR，清理重复、过时或误导内容，完成链接、索引、重复内容和状态检查。

本工单必须把本计划标记为历史保留或按证据安全删除；不得删除 Contract、Summary、Cockpit Status、评审/发布证据或仍被引用的设计文档。由于本计划此前错误地在工单1之前关闭，工单1及其影响评估、后续新版本发布完成后，由本最终文档闭环补记最终 PR/Release/关闭证据；不得把历史版本当作工单1之后的最终版本。

## 五、计划级完成定义

本计划已满足完成条件：工单1补执行完成，受影响的发布工单完成，工单14计划记录已由本 Work Item 更新；每个已启动工单均具备 Contract v2、针对性验证、Summary、归档记录、PR 编号与合并 SHA、`ai-close-work-item` 成功记录、远端/本地分支删除记录和主分支同步证据。历史版本仅作为补执行前的历史发布证据；v0.5.38 是工单1之后的新版本证据。
